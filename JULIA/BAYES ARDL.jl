...
lei_data = readtable(joinpath(in_dir, "LEI Data.csv"));

lei_growth = zeros(nrow(lei_data) - 1, ncol(lei_data) - 1);

i = 1
for i in 2:ncol(lei_data)
  lei_growth[:, i - 1] = diff(lei_data[i]) ./ lei_data[i][1:(end - 1)]
end

lei_df = DataFrame(lei_growth);
plot(lei_df, y = :x2, Geom.line)


indicators =  lei_df[:, collect(2:8)];
α² = 2
...
writetable(joinpath(ou_Rdir, "chain1.csv"), DataFrame(chain1))
"""
Plot it
"""
series = DataFrame(Y = y);
...
stepsize = 10;
# Training Data
est1 = mapslices(mean, chain1[(burnIn + 1):stepsize:end, :], [1]);
stde1 = mapslices(std, chain1[(burnIn + 1):stepsize:end, :], [1]);
...
for i in (burnIn + 1):stepsize:size(chain1)[1]
  ...
  for j in 1:(size(x)[2])
    yhat += chain1[i, j + 2] * x[:, j] + chain1[i, j + 2 + size(x)[2]] * x_1[:, j]
  end
...
series_stacked = DataFrame(
  x = repeat(collect(1:nrow(series)), outer = size(chain1[(burnIn + 1):stepsize:end, :])[1]),
...
for i in (burnIn + 1):stepsize:size(chain1)[1]
  yhat = chain1[i, 1] + chain1[i, 2] * y_1test
  for j in 1:(size(x)[2])
    yhat += chain1[i, j + 2] * x_test[:, j] + chain1[i, j + 2 + size(x)[2]] * x_1test[:, j]
  end
...
HMC_object2 = HMC(U, K, dU, dK, zeros(n_params, 1), n_params);

# Sample
@time chain2 = mcmc(HMC_object2, leapfrog_params = Dict([:ɛ => .0009, :τ => 20]), r = 100000);
