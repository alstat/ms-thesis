...
text3D(2.751, .8, 0, labels = expression(beta[1]), add = TRUE)
...
#beta0 <- seq(2.7, 2.74, length.out = 100)
#beta1 <- seq(.73, .87, length.out = 100)
z_val1 <- numeric()
g <- expand.grid(b0 = beta0, b1 = beta1)
beta <- t(as.matrix(g))
for (i in 1:ncol(beta)) {
...
c1
c2 <- coef(lm(y ~ x))[2]
levelplot(z ~ b0 * b1, g, col.regions = ramp.col(), region = TRUE, xlab = expression(beta[0]), ylab = expression(beta[1]),
          panel = function(x, y, z, ...) {
            panel.levelplot(x, y, z, ...)
            LOW <- p - 500
            UP <- p - 1
